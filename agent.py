...
def one_hot_encoding(board):
	ohe = np.zeros(28*2*6)
	ohe[board[np.where(board>5)]*5] = 1
	ohe[-board[np.where(board<-5)]*5*2] = 1

	for x in range(1,4):
		ohe[board[np.where(board == x)]*x] = 1
		ohe[-board[np.where(board == -x)]*x*2] = 1
    ...
    for games in range(0, numgames):
        ...
        for move in range(0, len(legal_moves)):
            ...
            if (player == tableplayer): # this one is using the table V
                possible_moves = Backgammon.legal_moves(board, dice, player)
            ...
            board[action] = player
            if (1 == Backgammon.game_over(board, player)): # has this player won?
                winner = player
            ...
            if tableplayer == player:
                ...
                xold = Variable(torch.tensor(one_hot_encoding(board, player), dtype=torch.float, device = device)).view(28*2*6,1)
            # swap players
            player = -player
...
beginning_board = Backgammon.init_board()
V = np.zeros(hash_it(beginning_board))
